...
const List = () => {
    ...
    useEffect(() => {
        const fetchEmployees = async () => {
            setEmpLoading(true)
            try {
                const response = await axios.get("https://employee-management-backend-flame.vercel.app/api/employees", {
                    headers: {
                ...
                if (response.data.success) {
                    ...
                    const data = response.data.employees.map((emp, index) => ({
                        ...
                        profileImage: <img width={40} className='rounded-full' src={`https://employee-management-backend-flame.vercel.app/${emp.userId.profileImage}`} />,
